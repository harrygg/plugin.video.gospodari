...
class Helper():
		...
		def get_categories(self, id):
			...
			if id == -4:
				...
				try:
					...
					res = Request('http://gospodari.com/mobile')
		...
		def get_video_info(self, url, isFullShow):
			...
			try:
				if isFullShow:
					url = 'http://playapi.mtgx.tv/v3/videos/stream/%s' % url 
					res = Request(url)
					data = json.loads(res)
					...
					if high != None:
						...
						if 'stream.novatv.bg' in medium:
							stream['url'] = medium.replace('rtmp://stream.novatv.bg/mediacache/mp4:http/', 'http://videobg.novatv.bg/')
						elif 'host.bg' in medium:
							stream['url'] = medium.replace('rtmp://stream.by.host.bg/mediacache/flv:http/', 'http://videobg.novatv.bg/')
					elif hls != None:
						stream['url'] = hls
					stream['date'] = ''
				else:
